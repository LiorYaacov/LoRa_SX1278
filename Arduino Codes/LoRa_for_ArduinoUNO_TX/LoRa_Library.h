...
const byte lora_PNSS = 1;//8	//pin number where the NSS line for the LoRa device is connected.
const byte lora_PLED1 = 7; //     //pin number for LED on RFM98 LoRa Shield
const byte lora_PReset = 2;//9	//pin where LoRa device reset line is connected
const byte lora_PPWMCH = 9;//10    //pin number for tone generation, connects to LoRa device DIO2.
...
void lora_Setup()
{
  ...
  lora_Write(lora_RegOpMode, 0x89);
...
void lora_FillTX()
{
  ...
  lora_TXStart = 0;
  char lior[4]; lior[0] = 'L'; lior[1] = 'i'; 
				lior[2] = 'o'; lior[3] = 'r'; 
  for (lora_LLoop = 0; lora_LLoop <= 3; lora_LLoop++)
  {
    lora_TXEnd++;
    lora_TXBUFF[lora_TXEnd] = lior[lora_LLoop];
  }
